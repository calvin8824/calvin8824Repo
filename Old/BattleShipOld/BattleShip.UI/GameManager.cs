...
");
        ...
        //Todo: If bored, combine choosing a direction and space.
        private void PlaceShips(Player player)
        {
            ...
            do
            {
                ...
                orientation = RequestOrientation(player, prompt);
                var request = new PlaceShipRequest
                {
                    Direction = orientation,
                    Coordinate = RequestCoordinate(player, prompt, 2, orientation),
            ...
            do
            {
                ...
                {
                    Direction = orientation,
                    Coordinate = RequestCoordinate(player, prompt, 3, orientation),
            ...
            do
            {
                ...
                {
                    Direction = orientation,
                    Coordinate = RequestCoordinate(player, prompt, 4, orientation),
            ...
            do
            {
                ...
                {
                    Direction = orientation,
                    Coordinate = RequestCoordinate(player, prompt, 5, orientation),
        ...
        private Coordinate RequestCoordinate(Player player,  string prompt, int size, ShipDirection orientation)
        {
            int y = 5;
            int x = 5;
            bool valid = false;
            ...
            do
            {
                PlayerShipDisplay(player, prompt, new Coordinate(x, y), size, orientation);
                ...
                if (keyInfo.Key == ConsoleKey.UpArrow)
                {
                    if (x > 1)
                    {
                        if (CheckShipArea(x - 1, y, orientation, size))
                        {
                            x--;
                        }
                    }
                ...
                else if (keyInfo.Key == ConsoleKey.LeftArrow)
                {
                    if (y > 1)
                    {
                        if (CheckShipArea(x, y - 1, orientation, size))
                        {
                            y--;
                        }
                    }
